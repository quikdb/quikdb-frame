...
| CLI Deploy Commands | As-is Git path released; v0.1.16 adds offline local archive packaging and capability-gated submission; native discovery blocks unsupported root-context/worker submissions | Activate only after immutable API/runner qualification; root-context and lifecycle parity |
| CLI Convert Commands | v0.1.16 `express-static-v1`: offline qualification/dry-run, explicit apply, deterministic native artifact and as-is rollback; no handler stubs | Expand only with independent preservation fixtures; live upload remains capability-gated |
...
| Converter Process/Mapping | Released strict pilot accepts only fixed literal Express routes and static mounts; every unrecognized construct fails closed | Adversarial fixture expansion before broadening the matrix |
| Supported Source Frameworks | Express 4.21.2/Node 20 static-response pilot only; every other framework remains as-is | Add frameworks only after independent original/target certification |
